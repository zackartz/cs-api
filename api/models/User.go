...
func (u *User) Find(db *gorm.DB, limit int) ([]User, error) {
	var err error
	var users []User
...
func (u *User) Delete(db *gorm.DB, uid uint32, rid uint32) (int64, error) {
	err := db.Debug().Model(&User{}).Where("id = ?", uid).Take(&u).Error
	if err != nil {
		return 0, err
	}
...
func (u *User) IsAdmin(db *gorm.DB, uid uint32) (bool, error) {
	err := db.Debug().Model(&User{}).Where("id = ?", uid).Take(&u).Error
	if err != nil {
		return false, err
	}
